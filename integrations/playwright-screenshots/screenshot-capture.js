...
async function capture(pageSpec, outDir) {
  ...
  try {
    ...
    const safeName = pageSpec.name.replace(/[^a-z0-9-_.]/gi, '_');
    const ext = type === 'jpeg' ? 'jpg' : 'png';
    const filename = `${safeName}.${ext}`;
    const outPath = path.join(outDir, filename);
    const screenshotOptions = { path: outPath };
    if (type === 'jpeg') screenshotOptions.type = 'jpeg';
    if (type === 'jpeg' && quality) screenshotOptions.quality = quality;
...
async function main() {
  ...
  for (const p of pages) {
    try {
      ...
      console.log('Captured', r.file, 'thumb', thumbPath);
    } catch (err) {
      console.error('Failed capture for', p.name, err.message);
    }
  }
  const meta = {
